...
def calculateAge(birthDate,visitday): 
    age = visitday.year - birthDate.year - ((visitday.month, visitday.day) < (birthDate.month, birthDate.day)) 
  
    return age 

def read_patients_table(mimic3_path):
    #print(mimic3_path)
    #pats = pd.read_csv(mimic3_path, header=0, index_col=0)
    pats = dataframe_from_csv(os.path.join(mimic3_path, 'PATIENTS.csv'))
    print(pats.head(2))
    pats = pats[['SUBJECT_ID', 'GENDER', 'DOB', 'DOD']]
    pats.DOB = [parse(x) for x in pats.DOB ]  #pd.to_datetime(pats.DOB).dt.date 
...
def read_admissions_table(mimic3_path):
    ...
    admits.ADMITTIME = [parse(x) for x in admits.ADMITTIME ]
    admits.DISCHTIME = [parse(x) for x in admits.DISCHTIME ]
    admits.DEATHTIME = pd.to_datetime(admits.DEATHTIME)  #[parse(x) for x in admits.DEATHTIME ]
    return admits


def read_icustays_table(mimic3_path):
    stays = dataframe_from_csv(os.path.join(mimic3_path, 'ICUSTAYS.csv'))
    stays.INTIME = [parse(x) for x in stays.INTIME ] 
...
def add_age_to_icustays(stays):
    ...
    #stays['INTIME'] = pd.to_datetime(stays['INTIME'], infer_datetime_format = True, errors = 'coerce')
    #print(stays.INTIME.year)
    #stays['DOB'] = pd.to_datetime(stays['DOB'], infer_datetime_format = True, errors = 'coerce')
    print("######")
    #print(stays.DOB.year)
    #print(stays.INTIME - stays.DOB)
    stays['AGE'] = stays.apply(lambda row: calculateAge(row['DOB'], row['INTIME']), axis=1)
    print(stays.AGE) 
