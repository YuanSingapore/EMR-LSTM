...
def add_age_to_icustays(stays):
    ...
    stays['INTIME'] = pd.to_datetime(pd.to_datetime(stays['INTIME']).dt.date, infer_datetime_format = True, errors = 'coerce')
    print(stays.INTIME)
    stays['DOB'] = pd.to_datetime(stays['DOB'], infer_datetime_format = True, errors = 'coerce').dt.date
    print("######")
    print(stays.DOB)
    #print(stays.INTIME - stays.DOB)
    stays['AGE1'] = stays.INTIME - stays.DOB
    print(stays.AGE) 
